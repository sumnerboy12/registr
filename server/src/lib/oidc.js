...
// Lazily discovered and cached — an unconfigured server shouldn't fail at
// startup, only once someone actually tries to sign in.
...
function getConfig() {
  ...
  if (!configPromise) {
    ...
    // non-TLS provider) — a real https:// issuer never needs this.
...
export async function buildAuthorizationUrl() {
  ...
  const url = client.buildAuthorizationUrl(config, {
    ...
    // whatever account is already cached — this forces the account chooser.
